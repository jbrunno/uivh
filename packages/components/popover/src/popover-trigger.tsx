...
const PopoverTrigger = forwardRef<"button", PopoverTriggerProps>((props, _) => {
  ...
  // validates if contains a VhsysUI Button as a child
  const [, triggerChildren] = pickChildren(children, Button);

  const {buttonProps} = useAriaButton({onPress}, triggerRef);

  const hasVhsysUIButton = useMemo<boolean>(() => {
    return triggerChildren?.[0] !== undefined;
  }, [triggerChildren]);

  return cloneElement(child, mergeProps(rest, hasVhsysUIButton ? {onPress} : buttonProps));
});

PopoverTrigger.displayName = "VhsysUI.PopoverTrigger";
