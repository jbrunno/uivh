...
if (process.env.IS_VERCEL_ENV !== "true") {
  console.log("Skipping preinstall script");
  process.exit(0);
}
...
fs.readFile("./package.json", "utf8", function (err, data) {
  ...
  Object.keys(packageJson.dependencies).forEach((pkg) => {
    // Check if the package is in the @vhsys-ui namespace and has "workspace:*" as its version
    if (pkg.startsWith("@vhsys-ui/") && packageJson.dependencies[pkg] === "workspace:*") {
