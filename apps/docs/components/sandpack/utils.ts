...
export function getVhsysUIComponents(files: SandpackFiles): {[filePath: string]: string} {
  ...
  for (const filePath in files) {
    ...
    const importStatements = extractVhsysUIImport(code);
...
export function extractVhsysUIImport(code: string): string[] {
  // Split the code into lines
  const lines = code.split("\n");

  // Filter the lines to only include import statements from "@vhsys-ui/react"
  const importStatements = lines.filter(
    (line) => line.startsWith("import") && line.includes("@vhsys-ui/react"),
  );
...
export function updateTailwindConfig(tailwindConfig: string, componentNames: string[]): string {
  ...
  const lineIndex = lines.findIndex((line) =>
    line.includes("./node_modules/@vhsys-ui/theme/dist/**/*.{js,ts,jsx,tsx}"),
  );
  ...
  if (lineIndex !== -1) {
    ...
    componentNames.forEach((componentName) => {
      ...
      lines.splice(
        lineIndex,
        0,
        `  "./node_modules/@vhsys-ui/theme/dist/components/${name}.js",`,
      );
    });
  }
